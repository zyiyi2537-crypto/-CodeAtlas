...
from .models import ApiToken, DocumentChunkRecord, WikiPage
...
def build_mcp(
    ...
    mcp = FastMCP(
        "CodeAtlas",
        instructions=(
            "Search the indexed demo repositories. Start with search_code or grep_code, "
    ...
    def search_documents(query: str, collection: str | None = None) -> list[dict]:
        ...
        with Session(engine) as database:
            ...
            if collection:
                statement = statement.where(DocumentChunkRecord.collection_id == collection)
            rows = database.exec(statement).all()
        ...
        for row in rows:
            ...
            if score:
                results.append({
                    "source_type": "document", "document_id": row.document_id,
                    "collection_id": row.collection_id, "title": row.title,
    ...
    def search_wiki(query: str) -> list[dict]:
        ...
        identity("read")
        with Session(engine) as database:
            pages = database.exec(select(WikiPage).where(WikiPage.status == "published")).all()
        terms = [term.lower() for term in query.split() if term.strip()]
        results = []
        for page in pages:
            ...
            if score:
                results.append({
                    "source_type": "wiki", "path": page.path, "title": page.title,
                    "content": page.content, "sources": json.loads(page.sources_json),
                    "score": score,
                })
        return sorted(results, key=lambda item: int(str(item["score"])), reverse=True)[:10]
    ...
    def get_wiki_page(path: str) -> dict:
        ...
        with Session(engine) as database:
            page = database.exec(
                select(WikiPage).where(WikiPage.path == path, WikiPage.status == "published")
            ).first()
        if not page:
