...
### GitHub Actions and protected delivery

GitHub Actions is a quality gate only: it runs backend, frontend, blog, and
secret-scanning checks. It does not hold production SSH credentials and does not
deploy automatically. Production releases are assembled from the exact tested
Git commit and transferred through a separately authenticated, operator-approved
maintenance session.
